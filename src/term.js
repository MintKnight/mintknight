...
class Prompt {
  ...
  static async contract () {
    ...
    if (answers.name === undefined || answers.symbol === undefined || answers.contractType === undefined) process.exit()
    return answers
  }
  ...
  static async attribute () {
    ...
    if (answers.trait_type === undefined || answers.value === undefined) process.exit()
  ...
  static async nft () {
    ...
    const answers = await prompt(questions, { onCancel: cleanup, onSubmit: cleanup })
    if (answers.name === undefined || answers.description === undefined) process.exit()
    answers.attributes = []
    let attribute = true
