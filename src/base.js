    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (parameters[keys[i]]) {
          ...
          // Check length.
          if (parameters[keys[i]].min && (params[keys[i]].length < parameters[keys[i]].min)) { reject(new Error(`Parameter ${keys[i]} should be at least ${parameters[keys[i]].min} caharcters`)) }
          // Check Valid values.
          if (parameters[keys[i]].valid && !parameters[keys[i]].valid.includes(params[keys[i]])) { reject(new Error(`Parameter ${keys[i]} is not valid : ${parameters[keys[i]].valid}`)) }
        }
      }
      resolve(params)
    })
