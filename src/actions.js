...
class Actions {
  ...
  static async register (nconf) {
    ...
    if (registeredUser !== undefined) error('Already registered. A config file already exists')
  ...
  static async mint (nconf) {
    ...
    const task = await service.mintNFT(contractId, minterId, minter.skey, walletId, nft)
    await service.waitTask(task.taskId)
    log('NFT Minted')
  }
